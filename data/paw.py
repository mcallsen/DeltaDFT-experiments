...
def get_pseudos_from_structure( structure, familyName ):

    from aiida.common.exceptions import NotExistent, MultipleObjectsError

#    paws = [ PawData.load_paw( family = familyName, symbol = kind.symbol)[0] for kind in structure.kinds ]
    ...
    for kind in structure.kinds:
        try:
            paws[ kind.symbol ] = PawData.load_paw( family = familyName, symbol = kind.symbol )[0]
        except KeyError:
            raise NotExistent("No PAW for element {} found in family {}".format(
                symbol, familyName))

    for kind, paw in paws.iteritems():
        result_paws[(kind,)] = paw
